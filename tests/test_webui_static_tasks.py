...
class WebUIStaticTaskTests(WebUIStaticTestCase):
    ...
    def test_task_history_anchor_navigation_feature_has_typescript_source_contract(self) -> None:
        ...
        self.assertRegex(
            html,
            r'id="taskActiveList"[\s\S]*id="taskHistoryTopAnchors"[\s\S]*id="taskList"[\s\S]*id="taskHistoryBottomAnchors"',
        )
    ...
    def test_task_list_queue_controls_delegate_queue_actions(self) -> None:
        ...
        self.assertIn("handleQueueDrop", controls_source)
    ...
    def test_running_preview_shows_partial_outputs(self) -> None:
        ...
        self.assertRegex(styles, r"\.elapsed-wheel-strip\s*\{[^}]*0\.12s")
    ...
    def test_elapsed_labels_tick_in_tenths_without_rebuilding_preview_images(self) -> None:
        ...
        self.assertIn("const root = els.taskHistoryShell || els.taskList", script)
        self.assertIn('root.querySelectorAll("[data-task-status-id]")', script)
        self.assertIn('setAttribute("aria-label", taskStatusAccessibleLabel(task))', script)
